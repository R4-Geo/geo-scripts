...
def ExportQgis(parameters):
    try:
        ...
        aois = sorted(
                        [[_nameZona(c[0]), int(c[0]), c[1]] for c in arcpy.da.SearchCursor(aoi, [field_clip] + ['Comuna'])],
                        key=lambda x: x[1]
                    )
            ...
            for c in feature_export:
                ...
                if count:
                    ...
                    if not sdf.empty:
                        ...
                        Export(sdf,shape_export,c,logs)
                        ...
                        if 'Id' in cols_:
                            arcpy.DeleteField_management(shape_export,'Id')
